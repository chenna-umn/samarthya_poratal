    ...
    <div class="container">   
        ...
        <div id="navbar" class="navbar-collapse collapse">
            <ul class="nav navbar-nav">
                <li class="<?php if (Yii::app()->user->getState("menu") == "home") { ?>active <?php } ?>" ><a style="color:#FFF" href="<?php echo Yii::app()->request->baseUrl . '/site/index'; ?>">Home</a></li>
            ...
            <ul class="nav navbar-nav navbar-right visible-sm visible-md visible-xs hidden-lg">                    
                    <li><a href="javascript:void(0);" data-toggle="modal" data-target="#myModal3">Login</a></li>
                    <li><a href="javascript:void(0);" data-toggle="modal" data-target="#myModal">Sign Up</a></li>
                </ul>
        </div>
    </div>
</nav>
<div class="modal fade" id="myModal3" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="background-color: rgba(250,250, 250, 0.5);">

    <div class="modal-dialog" style="top:25%;width: 40%;">
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="background-color: rgba(250,250, 250, 0.6);">
                    ...
                    <div id="myTabContent" class="tab-content">
                        ...
                        <div id="tab-step-2" class="container-fluid" style="display:none">
                            <div class="form-group">
                                ...
                                <input type="text" id="otpNumber"><button type="submit" class="btn btn-default pull-right" onclick="RegStep2()">Submit</button>
                            </div>
                            <div class="form-group">
                        ...
                        <div id="tab-step-3" class="container-fluid" style="display:none">
                            ...
                            <div class="col-xs-6" >
                                <div class="form-group">
                                    <input class="form-control" type="text" id="signup_username" onkeyup="checkUserNameAvail(this.value)" placeholder="Please Enter User Name" required>
                            ...
                            <br>
                            <div class="form-group">
                                <button type="submit" id="submitBttnFinish" class="btn btn-default pull-right" onclick="RegStep3()" style="display:none;">Confirm</button>
                                <button type="button"  class="btn btn-default pull-right" onclick="cancelReg()" style="margin-left:10px;margin-right:10px;">Cancel</button>
                            </div><br>
...
<script type="text/javascript">
    $('#passwordConfirm').on('keyup', function () {         
        if ($(this).val() == $('#password').val()) {
            ...
            $('#message').html('Sorry...!Passwords are not patching...Try again' ).css('color', 'red');
        ...
        if (userName.length >= 6) {
            jQuery.ajax({                            
                ...
                error: function(){
                    ...
                    $('#messageusername').html('Please enter Valid Details check availability for Registration').css('color', 'red');
                },
                success: function(resp){    
                    ...
                    if(resp=="registered"){
                        ...
                        $('#messageusername').html('User already Exists with entered username...Please Try with another username').css('color', 'red');                   
                    }else{
                        document.getElementById("submitBttnFinish").style.display='';
                        document.getElementById("signup-status").style.display = '';
                        $('#messageusername').html('This username Is Available to register with us').css('color', 'green'); 
...
<script type="text/javascript">
                ...
                success: function(resp){    
                    ...
                    if(resp=='alreadyRegistered'){
                        ...
                        if(json.Error_Code !=""){
                            ...
                            if(json.Error_Code =="NREGA-OLP-021"){
                                $('#mobile-valid').html('<?php echo Yii::app()->params['NREGA-OLP-021']; ?>').css('color', 'red');
                            }else if(json.Error_Code =="NREGA-OLP-022"){
                                $('#mobile-valid').html('<?php echo Yii::app()->params['NREGA-OLP-022']; ?>').css('color', 'red');
                            }else if(json.Error_Code =="NREGA-OLP-023"){
                                $('#mobile-valid').html('<?php echo Yii::app()->params['NREGA-OLP-023']; ?>').css('color', 'red');
                            }else if(json.Error_Code =="NREGA-OLP-024"){
                                $('#mobile-valid').html('<?php echo Yii::app()->params['NREGA-OLP-024']; ?>').css('color', 'red');
                            }else if(json.Error_Code =="NREGA-OLP-025"){
                                $('#mobile-valid').html('<?php echo Yii::app()->params['NREGA-OLP-025']; ?>').css('color', 'red');
                            }else{
                                $('#mobile-valid').html('<?php echo Yii::app()->params['NREGA-OLP-ERRMBL']; ?>').css('color', 'red');
                            }
            ...
            jQuery.ajax({                            
                ...
                success: function(resp){
                    ...
                    if(resp=='alreadyRegistered'){
                        ...
                        if(json.Error_Code !=""){
                            document.getElementById("mobilevalid").style.display = '';
                           if(json.Error_Code =="NREGA-OLP-011"){
                                $('#mobile-valid').html('<?php echo Yii::app()->params['NREGA-OLP-021']; ?>').css('color', 'red');
                            }else if(json.Error_Code =="NREGA-OLP-012"){
                                $('#mobile-valid').html('<?php echo Yii::app()->params['NREGA-OLP-022']; ?>').css('color', 'red');
                            }else if(json.Error_Code =="NREGA-OLP-013"){
                                $('#mobile-valid').html('<?php echo Yii::app()->params['NREGA-OLP-023']; ?>').css('color', 'red');
                            }else if(json.Error_Code =="NREGA-OLP-014"){
                                $('#mobile-valid').html('<?php echo Yii::app()->params['NREGA-OLP-024']; ?>').css('color', 'red');
                            }else if(json.Error_Code =="NREGA-OLP-015"){
                                $('#mobile-valid').html('<?php echo Yii::app()->params['NREGA-OLP-025']; ?>').css('color', 'red');
                            }else{
                                $('#mobile-valid').html('<?php echo Yii::app()->params['NREGA-OLP-ERRADR']; ?>').css('color', 'red');
                            }
                        }else{                         
                            userdata=json.Response_Data;
                            $('#btnstep2').removeClass('btn-primary');
    ...
    {                  
        ...
        if (mobile.length == 10) {
            document.getElementById("mobilevalid").style.display = '';
            $('#mobile-valid').html('You Entered  Valid Mobile number to Register' ).css('color', 'green');
        } else{
            document.getElementById("mobilevalid").style.display = '';
    ...
    {                  
        ...
        if (aadhar.length == 12) {
            document.getElementById("mobilevalid").style.display = '';
            $('#mobile-valid').html('You Entered  Valid Aadhaar number to Register' ).css('color', 'green');
        } else{
            document.getElementById("mobilevalid").style.display = '';
